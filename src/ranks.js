...
export function getProgress(points) {
  ...
  if (!next) {
    return { percent: 100, label: 'Grade maximum atteint' }
  }
  ...
  return { percent, label: `${remaining} pts avant ${next.name}` }
}
...
export function getMedals(player) {
  ...
  if (player.rank === 1) medals.push({ icon: '🥇', label: '1ère place' })
  else if (player.rank === 2) medals.push({ icon: '🥈', label: '2ème place' })
  else if (player.rank === 3) medals.push({ icon: '🥉', label: '3ème place' })

  if (getRankForPoints(player.points).name === 'Special Grade') {
    medals.push({ icon: '👑', label: 'Special Grade' })
  }

  if (player.wins >= 100) medals.push({ icon: '🏆', label: '100+ victoires' })
  else if (player.wins >= 50) medals.push({ icon: '🎖️', label: '50+ victoires' })
  else if (player.wins >= 10) medals.push({ icon: '⭐', label: '10+ victoires' })

  if (wrTotal >= 10 && winrate >= 0.7) {
    medals.push({ icon: '🔥', label: 'Winrate 70%+' })
  }
